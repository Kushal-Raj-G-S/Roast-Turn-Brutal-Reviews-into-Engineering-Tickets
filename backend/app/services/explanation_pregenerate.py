...
def _format_structured_rca(agent_result: dict) -> tuple[str, str, str]:
    """
    Flattens the agent's StructuredRCA + evaluation + precedent into the
    existing three free-text DB columns (rca_hypothesis / rca_steps / rca_fix)
    -- no schema migration needed, and the existing frontend already renders
    these as-is.
    """
    final = agent_result["final_rca"]
    h = final.hypothesis

    similar = agent_result.get("similar_issues") or []
    precedent_lines = "\n".join(
        f'  - "{s.title}" ({s.severity}, {s.status})' for s in similar
    ) or "  - None found in project history"

    eval_scores = agent_result.get("eval_scores")
    eval_line = (
        f"\n\n**AI Quality Score** (RAGAS) -- faithfulness: {eval_scores['faithfulness']}, "
        f"answer relevancy: {eval_scores['answer_relevancy']}"
        if eval_scores else ""
    )

    hypothesis_text = (
        f"{h.explanation}\n\n"
        f"**Likelihood:** {h.likelihood} - **Scope:** {h.scope}\n"
        f"**Suggested severity:** {h.suggested_severity} ({h.severity_reason})\n\n"
        f"**Similar issues found (hybrid search + reranking):**\n{precedent_lines}"
        f"{eval_line}\n\n"
        f"_Generated by a LangGraph agent (investigate -> retrieve similar -> hypothesize -> "
        f"critique -> finalize), confidence {final.confidence:.0%}._"
    )

    def _strip_leading_number(s: str) -> str:
        # The model sometimes numbers its own steps; avoid "1. 1. do the thing".
        return re.sub(r"^\s*\d+[.)]\s*", "", s).strip()

    steps_text = (
        "**Reproduction steps:**\n"
        + "\n".join(f"{i}. {_strip_leading_number(s)}" for i, s in enumerate(final.reproduction_steps, 1))
        + "\n\n**Diagnostic checklist:**\n"
        + "\n".join(f"- {s}" for s in final.diagnostic_checklist)
    )

    fix_text = f"**Suggested fix:**\n{final.suggested_fix}\n\n**Prevention:**\n{final.prevention}"
    if final.notes:
        fix_text += f"\n\n**Notes:**\n{final.notes}"

    return hypothesis_text, steps_text, fix_text


def _build_ai_metadata(agent_result: dict) -> dict:
    """
    Structured (non-markdown) record of what the agent actually did, for the
    frontend to render as real badges/meters/chips instead of parsing prose.
    """
    final = agent_result["final_rca"]
    h = final.hypothesis
    similar = agent_result.get("similar_issues") or []
    ...
    return {
        ...
        "scope": h.scope,
        "suggested_severity": h.suggested_severity,
        "severity_reason": h.severity_reason,
        "confidence": final.confidence,
        "similar_issues": [
            {"cluster_id": s.cluster_id, "title": s.title, "severity": s.severity, "status": s.status}
            for s in similar
        ],
        "eval_scores": agent_result.get("eval_scores"),
        "trace_id": agent_result.get("trace_id"),
        "agent_steps": ["investigate", "retrieve_similar", "hypothesize", "critique", "finalize"],
    }
...
async def _generate_rca_for_one_cluster(
    ...
    """
    Generate and persist an RCA for a single cluster via the agentic pipeline
    (LangGraph + hybrid-search precedent retrieval + structured output +
    RAGAS evaluation). Falls back to the plain single-shot prompt if the
    agent pipeline fails for any reason -- RCA generation should never go
    ...
    try:
        ...
        logger.info(
            f"[rca] Running RCA agent for cluster {cluster_id} "
            f"({cluster.title[:60]}... | {cluster.review_count} reviews)"
        )

        hypothesis_text = steps_text = fix_text = None
        ai_metadata = None

        try:
            from app.services.ai.rca_agent import run_rca_agent
            from app.services.ai.evaluation import evaluate_rca

            agent_result = await run_rca_agent(
                cluster_id=cluster.id,
                title=_clean_title(cluster.title),
                severity=cluster.severity,
                app_name=app_name,
                platform=_detect_platform(cluster),
                reviews=cluster.sample_reviews or [],
                keywords=cluster.keywords,
            )

            if agent_result.get("final_rca") is not None:
                review_contexts = [
                    r.get("content", "") for r in (cluster.sample_reviews or []) if r.get("content")
                ]
                agent_result["eval_scores"] = await evaluate_rca(
                    issue_title=_clean_title(cluster.title),
                    hypothesis_text=agent_result["final_rca"].hypothesis.explanation,
                    review_contexts=review_contexts,
                )
                hypothesis_text, steps_text, fix_text = _format_structured_rca(agent_result)
                ai_metadata = _build_ai_metadata(agent_result)
                logger.info(
                    f"[rca] Agent pipeline succeeded for cluster {cluster_id} "
                    f"(trace {agent_result['trace_id']}, {len(agent_result['similar_issues'])} precedents)"
                )
        except Exception as agent_err:
            logger.warning(f"[rca] Agent pipeline failed for cluster {cluster_id} ({agent_err}) -- falling back to single-shot RCA")

        if hypothesis_text is None:
            prompt = _build_rca_prompt(cluster, app_name)
            hypothesis_text = (await llm.generate(prompt, max_tokens=MAX_TOKENS_RCA)).strip()

        with Session(engine) as session:
            cluster = session.get(Cluster, cluster_id)
            if cluster:
                cluster.rca_hypothesis = hypothesis_text
                if steps_text:
                    cluster.rca_steps = steps_text
                if fix_text:
                    cluster.rca_fix = fix_text
                if ai_metadata:
                    cluster.ai_metadata = ai_metadata
        ...
        # Index into the vector store for future "have we seen this before" lookups --
        # best-effort, never blocks RCA persistence.
        try:
            from app.services.ai import vector_store
            with Session(engine) as session:
                fresh_cluster = session.get(Cluster, cluster_id)
                if fresh_cluster:
                    vector_store.index_cluster(fresh_cluster)
        except Exception as index_err:
            logger.warning(f"[rca] Vector store indexing failed for cluster {cluster_id} (non-fatal): {index_err}")
